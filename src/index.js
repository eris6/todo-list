...
//Get today's date
let currentDate = format(new Date(), "EEEE, MMMM dd");

const addOneDay = format(addDays(new Date(), 1), "EEEE, MMMM dd");

const addTwoDays = format(addDays(new Date(), 3), "EEEE, MMMM dd");
...
const defaultTaskOne = toDo(
  "Identify the planets",
  "Research the planets visible tonight and check which ones you can spot with the naked eye. Mark them on a stargazing map.",
  addOneDay,
  "HIGH",
  false,
);
const defaultTaskTwo = toDo(
  "Learn the Phases of the Moon",
  "Study the different phases of the moon. Track the current phase and update your lunar calendar.",
  currentDate,
  "LOW",
  false,
);
const defaultTaskThree = toDo(
  "Spot a Constellation",
  " Pick a constellation to learn about. Use a stargazing app to help you locate it in the sky tonight.",
  addTwoDays,
  "MEDIUM",
  false,
);
...
if (localStorage.length === 0) {
  ...
  if (projectListObj) {
    for (let i = 0; i < projectListObj.length; i++) {
      ...
      if (
        projectListObj[i].name !== "All Tasks" &&
        projectListObj[i].name !== "Today" &&
        projectListObj[i].name !== "Upcoming" &&
        projectListObj[i].name !== "Completed"
      ) {
      ...
      for (let i = 0; i < currTaskList.length; i++) {
        const currTask = toDo(
          currTaskList[i].title,
          currTaskList[i].description,
          currTaskList[i].dueDate,
          currTaskList[i].priority,
          currTaskList[i].completed,
        );
